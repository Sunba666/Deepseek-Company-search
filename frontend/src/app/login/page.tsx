    ...
    <div className="w-full max-w-md">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-gray-500">
          {isLogin ? "登录以继续管理你的求职进度" : "注册以开始你的 AI 求职之旅"}
        </p>
      </div>

      {/* OAuth Buttons */}
      <div className="flex gap-3 mb-6">
        <button className="flex-1 flex items-center justify-center gap-2 h-11 rounded-xl border border-default hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm font-medium">
      ...
      <div className="relative mb-6">
        ...
        <div className="relative flex justify-center text-xs"><span className="bg-primary px-2 text-gray-500">或使用邮箱</span></div>
      </div>
        ...
        {isLogin && (
          <div className="flex items-center justify-between">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="rounded border-gray-300" /> 记住我
            </label>
      ...
      <p className="text-center text-sm text-gray-500 mt-6">
        {isLogin ? "还没有账号？" : "已有账号？"}
        <button onClick={() => setIsLogin(!isLogin)} className="text-primary-500 hover:text-primary-600 font-medium ml-1">
          {isLogin ? "立即注册" : "立即登录"}
        </button>
      </p>
    </div>
