...
def generate_mock_company_data(company_name):
    """
    生成模拟公司数据。从 data/mock_companies.json 读取，匹配公司关键词或回退到 BRAND_MAPPING。
    :param company_name: 公司名称
    :return: 公司数据字典
    """
    # 缓存 JSON 数据到函数属性，避免每次重读
    _cache = getattr(generate_mock_company_data, '_cache', None)
    if _cache is None:
        import json, os
        _path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'mock_companies.json')
        with open(_path, 'r', encoding='utf-8') as f:
            generate_mock_company_data._cache = json.load(f)['company_keywords']
        _cache = generate_mock_company_data._cache

    # 查找匹配的公司数据
    for keyword, data in _cache.items():
        ...
        for brand, full_name in BRAND_MAPPING.items():
            if brand.lower() in company_name.lower() or company_name.lower() in brand.lower():
                base_data = {"name": full_name, "data_source": "品牌匹配"}
                matched = True
                break
        if not matched:
        ...
        if not matched:
            base_data = {
                "name": company_name, "data_source": "用户输入",
                "invalid_entity": True,
